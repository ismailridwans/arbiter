...
function simulateBuy(book, shares) {
  return walk(book.asks, shares);
}
function feeUsd(notionalUsd, takerFeeBps) {
  return notionalUsd * takerFeeBps / 1e4;
}
...
// src/demo.ts
function ob(tokenId, ask, size = 250) {
  return {
    tokenId,
    asks: [{ price: ask, size }],
...
function market(id, question, group, eventSlug, yes, yesTok, noTok, eventTitle = "") {
  return {
    id,
    slug: id,
    question,
...
function totalDepth(levels) {
  return levels.reduce((s, l) => s + l.size, 0);
}
function maxSharesPreservingEdgeMulti(askBooks, maxSumPrice, hardShareCap) {
...
// src/strategy/coherence-strategy.ts
var EMPTY_GRAPH = { nodes: /* @__PURE__ */ new Map(), implications: [], partitions: [] };
var CoherenceStrategy = class {
  constructor(client, risk2, opts) {
    this.client = client;
    this.risk = risk2;
    this.opts = opts;
  }
  ...
  async decide(edges, ctx) {
    const orders = [];
    const maxSumPrice = 1 - ctx.minEdge - ctx.takerFeeBps / 1e4;
...
var PaperBroker = class {
  constructor(client, takerFeeBps) {
    this.client = client;
...
async function demoData() {
  ...
  const stub = { fetchOrderBook: async (id) => books.get(id) };
  ...
  const sharesByEdge = /* @__PURE__ */ new Map();
