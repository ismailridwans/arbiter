...
var RawEnv = external_exports.object({
  LLM_PROVIDER: external_exports.enum(["groq", "gemini", "openrouter", "ollama", "none"]).default("none"),
  GROQ_API_KEY: external_exports.string().optional(),
  GEMINI_API_KEY: external_exports.string().optional(),
  OPENROUTER_API_KEY: external_exports.string().optional(),
  OLLAMA_BASE_URL: external_exports.string().url().default("http://localhost:11434"),
  LLM_MODEL: external_exports.string().optional(),
  POLYMARKET_GAMMA_URL: external_exports.string().url().default("https://gamma-api.polymarket.com"),
  POLYMARKET_CLOB_URL: external_exports.string().url().default("https://clob.polymarket.com"),
  MIN_EDGE: external_exports.coerce.number().min(0).max(1).default(0.02),
  KELLY_FRACTION: external_exports.coerce.number().min(0).max(1).default(0.25),
  MAX_STAKE_PER_LEG: external_exports.coerce.number().min(0).default(50),
  BANKROLL: external_exports.coerce.number().min(0).default(1e3),
  TAKER_FEE_BPS: external_exports.coerce.number().min(0).default(0),
  POLL_INTERVAL_MS: external_exports.coerce.number().min(1e3).default(15e3),
  EXECUTION_MODE: external_exports.enum(["paper", "live"]).default("paper")
});
var DEFAULT_MODELS = {
...
function loadConfig() {
  ...
  const enabled = env.LLM_PROVIDER !== "none" && (env.LLM_PROVIDER === "ollama" || Boolean(apiKey));
  const llm2 = {
    provider: env.LLM_PROVIDER,
    enabled,
    apiKey,
    baseUrl: env.LLM_PROVIDER === "ollama" ? env.OLLAMA_BASE_URL : void 0,
...
// src/polymarket/schema.ts
var StringifiedArray = external_exports.preprocess((v) => {
  if (typeof v === "string") {
    try {
...
}, external_exports.array(external_exports.union([external_exports.string(), external_exports.number()])).default([]));
var idLike = external_exports.union([external_exports.string(), external_exports.number()]).transform(String);
var GammaTagSchema = external_exports.object({ id: idLike.optional(), label: external_exports.string().optional(), slug: external_exports.string().optional() }).passthrough();
var GammaMarketSchema = external_exports.object({
  id: idLike,
  question: external_exports.string().default(""),
  slug: external_exports.string().default(""),
  description: external_exports.string().optional(),
  active: external_exports.boolean().default(false),
  closed: external_exports.boolean().default(true),
  endDate: external_exports.string().optional(),
  groupItemTitle: external_exports.string().optional(),
  outcomes: StringifiedArray,
  outcomePrices: StringifiedArray,
  clobTokenIds: StringifiedArray,
  volume: external_exports.coerce.number().optional(),
  liquidity: external_exports.coerce.number().optional()
}).passthrough();
var GammaEventSchema = external_exports.object({
  id: idLike,
  ...
  active: external_exports.boolean().default(false),
  closed: external_exports.boolean().default(true),
  tags: external_exports.array(GammaTagSchema).default([]),
  markets: external_exports.array(GammaMarketSchema).default([])
}).passthrough();
var ClobLevelSchema = external_exports.object({ price: external_exports.coerce.number(), size: external_exports.coerce.number() }).passthrough();
var ClobBookSchema = external_exports.object({
  asset_id: external_exports.string().optional(),
  market: external_exports.string().optional(),
  bids: external_exports.array(ClobLevelSchema).default([]),
  asks: external_exports.array(ClobLevelSchema).default([])
}).passthrough();
function normalizeMarket(m, event) {
  ...
  const tagLabels = (event?.tags ?? []).map((t) => t.label ?? t.slug ?? "").filter((s) => s.length > 0);
  return {
    id: m.id,
    slug: m.slug,
    question: m.question,
...
// src/polymarket/history.ts
var PriceHistorySchema = external_exports.object({
  history: external_exports.array(external_exports.object({ t: external_exports.coerce.number(), p: external_exports.coerce.number() })).default([])
});

// src/polymarket/client.ts
var TIMEOUT_MS = 12e3;
var EventsResponse = external_exports.array(GammaEventSchema).catch([]);
var PolymarketClient = class {
  constructor(gammaUrl, clobUrl) {
    this.gammaUrl = gammaUrl;
    this.clobUrl = clobUrl;
  }
  gammaUrl;
  clobUrl;
  async getJson(url, retries = 2) {
    let lastErr;
  ...
  async fetchEventsByTag(tagId, maxEvents) {
  ...
  async fetchPriceHistory(tokenId, fidelityMin) {
...
var KalshiMarketSchema = external_exports.object({
  ticker: external_exports.string(),
  yes_sub_title: external_exports.string().optional(),
  no_sub_title: external_exports.string().optional(),
  status: external_exports.string().optional(),
  yes_bid_dollars: external_exports.coerce.number().optional(),
  yes_ask_dollars: external_exports.coerce.number().optional(),
  no_bid_dollars: external_exports.coerce.number().optional(),
  no_ask_dollars: external_exports.coerce.number().optional(),
  last_price_dollars: external_exports.coerce.number().optional()
}).passthrough();
var KalshiMarketsResponse = external_exports.object({
  markets: external_exports.array(KalshiMarketSchema).default([]),
...
var KalshiClient = class {
   ...
   */
  async fetchChampionMarkets(seriesTicker = "KXNBA", seasonPrefix = "KXNBA-26-") {
...
function classifyMarket(m) {
  ...
  const conference = (team ? conferenceOf(team) : void 0) ?? (/eastern/.test(text) ? "East" : /western/.test(text) ? "West" : void 0);
  let kind = "other";
...
var LATTICE_KINDS = /* @__PURE__ */ new Set(["championship", "conference", "series"]);
function nodeKey(team, kind) {
  return `${team}:${kind}`;
}
function buildGraph(markets, classify = classifyMarket) {
  ...
  const implications = [];
  const addImpl = (team, sub, sup, why) => {
...
// src/coherence/detector.ts
var EMPTY_BOOK = { tokenId: "", asks: [], bids: [], timestamp: 0 };
function tokensToPrice(graph, minProb) {
...
var feeAdj = (risk2) => risk2.takerFeeBps / 1e4;
function detectEdges(graph, books, risk2) {
  const edges = [];
  const ts = Date.now();
  const ask = (tokenId) => tokenId ? bestAsk(books.get(tokenId) ?? EMPTY_BOOK) : void 0;
  for (const impl of graph.implications) {
    ...
    if (aNo === void 0 || aYes === void 0 || !subNo || !supYes) continue;
    const net = 1 - (aNo + aYes) - feeAdj(risk2);
    const legs = [
  ...
  const seenMarket = /* @__PURE__ */ new Set();
  for (const n of graph.nodes.values()) {
    ...
    if (aYes === void 0 || aNo === void 0 || !yes || !no) continue;
    const net = 1 - (aYes + aNo) - feeAdj(risk2);
    if (net <= 0) continue;
    edges.push({
      id: `comp:${n.market.id}`,
      type: "complementary",
  ...
  for (const part of graph.partitions) {
    const members = part.nodeKeys.map((k) => graph.nodes.get(k)).filter((x) => Boolean(x));
    const legs = [];
    let sum = 0;
    let priced = 0;
...
function partitionStats(graph) {
  return graph.partitions.map((p) => {
    const nodes = p.nodeKeys.map((k) => graph.nodes.get(k)).filter((x) => Boolean(x));
...
var OpenAiCompatLlm = class {
  constructor(label, baseUrl, model, apiKey) {
    this.label = label;
    this.baseUrl = baseUrl;
    this.model = model;
    this.apiKey = apiKey;
...
function createLlm(cfg2) {
  if (!cfg2.enabled) return new DisabledLlm();
  switch (cfg2.provider) {
    case "groq":
      return new OpenAiCompatLlm("groq", "https://api.groq.com/openai/v1", cfg2.model, cfg2.apiKey);
    case "openrouter":
      return new OpenAiCompatLlm("openrouter", "https://openrouter.ai/api/v1", cfg2.model, cfg2.apiKey);
    case "ollama":
      return new OpenAiCompatLlm("ollama", `${cfg2.baseUrl ?? "http://localhost:11434"}/v1`, cfg2.model);
    case "gemini":
      return new GeminiLlm(cfg2.model, cfg2.apiKey ?? "");
    default:
...
// src/ai/agents.ts
var ANALYST_SYS = "You are the Market Analyst agent for a market-neutral prediction-market arbitrage desk. You assess where coherence (logical price-consistency) edges exist. Be concise and concrete. No hype.";
var ARCHITECT_SYS = "You are the Strategy Architect agent. You choose which market-neutral coherence-arbitrage approaches to pursue and the risk posture. 2-3 sentences.";
var DEVELOPER_SYS = "You are the Developer agent. You translate the strategy into concrete order actions (which legs to buy), or state that none qualify. 2-3 sentences.";
var QA_SYS = "You are the QA agent. You validate risk before going live (net edge threshold, market-neutrality, exposure caps) and APPROVE or REJECT with a one-line reason. Start your reply with APPROVE or REJECT.";
function topNodes(graph, n) {
  return [...graph.nodes.values()].sort((a, b) => b.prob - a.prob).slice(0, n).map((node) => `${node.team} ${node.kind} ${(node.prob * 100).toFixed(1)}%`).join("; ");
}
function summarize(ctx) {
  const parts = ctx.partitions.map((p) => `${p.label} \u03A3=${p.sum.toFixed(3)} (overround ${p.overround >= 0 ? "+" : ""}${(p.overround * 100).toFixed(1)}%)`).join("; ");
  const tradeable = ctx.edges.filter((e) => e.netEdge >= ctx.minEdge);
  return `Markets scanned: ${ctx.marketCount}. Lattice nodes: ${ctx.graph.nodes.size}. Top: ${topNodes(ctx.graph, 5)}. Partitions: ${parts || "none"}. Tradeable edges (net \u2265 ${(ctx.minEdge * 100).toFixed(1)}%): ${tradeable.length}` + (tradeable[0] ? ` (best ${tradeable[0].type} ${(tradeable[0].netEdge * 100).toFixed(1)}%)` : "") + ".";
}
async function speak(llm2, role, system, prompt, fallback) {
  if (!llm2.enabled) return { role, content: fallback, source: "rule" };
  try {
    const text = (await llm2.complete(prompt, { system, maxTokens: 220 })).trim();
    return { role, content: text.length > 0 ? text : fallback, source: "llm" };
  } catch {
    return { role, content: fallback, source: "rule" };
  }
}
async function runAgentWorkflow(llm2, ctx) {
...
Give a 2-3 sentence opportunity assessment.`,
    `Scanned ${ctx.marketCount} markets into ${ctx.graph.nodes.size} lattice nodes. The championship field carries a ${overround >= 0 ? "+" : ""}${(overround * 100).toFixed(1)}% overround. ${tradeable.length} coherence edge(s) clear the ${minPct}% net bar; the rest is internally consistent at tradeable prices.`
  );
...
Recommend the approach.`,
    `Pursue market-neutral coherence arbitrage only \u2014 implication (champion \u2291 conference), complementary, and cross-venue (Polymarket\u2194Kalshi). Hold a strict net-edge floor of ${minPct}%, quarter-Kelly sizing, and per-leg caps. Never take a directional view on a game.`
  );
...
There are ${tradeable.length} tradeable edges. Describe the concrete orders to place, or state none.`,
    tradeable.length > 0 ? `Place ${tradeable.length} matched-leg basket(s); e.g. ${tradeable[0].legs.length}-leg ${tradeable[0].type} for ${(tradeable[0].netEdge * 100).toFixed(1)}% locked. Size each to the largest amount that preserves the edge after the live spread.` : `No edges clear the ${minPct}% floor right now \u2014 construct no orders and keep scanning every cycle.`
  );
  const approved = tradeable.every((e) => e.netEdge >= ctx.minEdge);
...
Validate risk: net edge \u2265 ${minPct}%, market-neutral, within caps. Approve or reject.`,
    tradeable.length > 0 ? `APPROVE \u2014 all ${tradeable.length} proposed legs are market-neutral with net edge \u2265 ${minPct}% after spread, and notional is within per-leg + bankroll caps.` : `APPROVE \u2014 stand down. The market is coherent; no orders, no capital at risk. Resume on the next dislocation.`
  );
  return { analyst, architect, developer, qa: { ...qaMsg, approved } };
}

// functions/_engine.ts
var cfg = loadConfig();
var pm = new PolymarketClient(cfg.polymarket.gammaUrl, cfg.polymarket.clobUrl);
var kalshi = new KalshiClient();
var llm = createLlm(cfg.llm);
var risk = {
