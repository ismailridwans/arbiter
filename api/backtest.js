...
// src/backtest/backtest.ts
function alignViolations(subHist, supHist, params) {
  ...
  for (const pt of sub) {
    ...
    if (Math.abs(cand.t - pt.t) > tolSec) continue;
    const supP = cand.p;
...
function buildPairs(markets) {
  ...
  for (const [key, market] of byKey) {
    ...
    const supToken = conf ? yesToken(conf)?.tokenId : void 0;
...
async function fetchPairHistories(client, fidelityMin) {
  const raw = (await Promise.all(SEED_EVENT_SLUGS.map((s) => client.fetchEventBySlug(s).catch(() => [])))).flat();
  const pairs = buildPairs(raw);
  return Promise.all(
    pairs.map(async (pair) => {
      const [subHist, supHist] = await Promise.all([
        client.fetchPriceHistory(pair.subToken, fidelityMin).catch(() => []),
        client.fetchPriceHistory(pair.supToken, fidelityMin).catch(() => [])
      ]);
      return { team: pair.team, relation: `${pair.subKind} \u2291 ${pair.supKind}`, subHist, supHist };
    })
  );
}
function aggregate(histories, params) {
  const trades = [];
  const byTeam = {};
