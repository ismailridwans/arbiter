...
function detectCrossVenue(matches) {
  return matches.map(crossVenueEdge).filter((e) => e !== null).sort((a, b) => b.netEdge - a.netEdge);
}
function matchByTeam(pm2, kalshi2) {
  const out = [];
  for (const k of kalshi2) {
    const p = pm2.get(k.team);
    if (!p) continue;
    out.push({
      team: k.team,
      pm: p,
...
// src/crossvenue/run.ts
async function runCrossVenue(pm2, kalshi2, opts) {
  const [pmMarkets, kalshiMarkets] = await Promise.all([
    pm2.fetchEventBySlug("2026-nba-champion"),
    kalshi2.fetchChampionMarkets()
  ]);
  const pmQuotes = /* @__PURE__ */ new Map();
  const bookFetches = [];
  for (const m of pmMarkets) {
    ...
    const no = noToken(m);
    const quote = { yesMid: yes?.price };
    pmQuotes.set(team, quote);
    if ((yes?.price ?? 0) >= opts.minProb && yes?.tokenId && no?.tokenId) {
      bookFetches.push(
        (async () => {
          const [yesBook, noBook] = await Promise.all([
            pm2.fetchOrderBook(yes.tokenId).catch(() => null),
            pm2.fetchOrderBook(no.tokenId).catch(() => null)
          ]);
...
async function handler(_req, res) {
  ...
  res.end(JSON.stringify(await crossVenueData()));
}
export {
  handler as default
};
